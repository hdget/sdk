...
import (
	...
	"github.com/hdget/sdk/libs/wechat/wx/thirdparty/cache"
	"github.com/hdget/sdk/libs/wechat/wx/thirdparty/wxapi"
...
type API interface {
	...
	GetAuthorizerInfo(appId string) (*wxapi.GetAuthorizerInfoResult, error) // 获取授权应用的信息
...
func (impl thirdPartyApiImpl) GetAuthUrl(client, redirectUrl string, authType int) (string, error) {
	...
	preAuthCode, _, err := impl.Api.CreatePreAuthCode(componentAccessToken)
	if err != nil {
		return "", err
	}
...
func (impl thirdPartyApiImpl) GetAuthorizerInfo(appId string) (*wxapi.GetAuthorizerInfoResult, error) {
	componentAccessToken, err := impl.getComponentAccessToken()
	if err != nil {
		return nil, err
	}
...
func (impl thirdPartyApiImpl) GetAuthorizerAccessToken(authorizerAppid string) (string, error) {
	return api.CacheFirst(
		...
		func() (string, int, error) {
			...
			return impl.Api.GetAuthorizerAccessToken(componentAccessToken, authorizerAppid, authorizerRefreshToken)
		},
	)
}
...
func (impl thirdPartyApiImpl) getAuthorizerRefreshToken(authorizerAppId string) (string, error) {
	return api.CacheFirst(
		...
		func() (string, int, error) { // wx api
			...
			result, err := wxapi.New(impl.GetAppId(), impl.GetAppSecret()).GetAuthorizerInfo(componentAccessToken, authorizerAppId)
			if err != nil {
...
func (impl thirdPartyApiImpl) getComponentVerifyTicket() (string, error) {
	...
	if componentVerifyTicket == "" {
		...
		if err := wxapi.New(impl.GetAppId(), impl.GetAppSecret()).StartPushComponentVerifyTicket(); err != nil {
...
func (impl thirdPartyApiImpl) getComponentAccessToken() (string, error) {
	return api.CacheFirst(
		...
		func() (string, int, error) {
			...
			return wxapi.New(impl.GetAppId(), impl.GetAppSecret()).GetComponentAccessToken(componentVerifyTicket)
		},
	)
}
