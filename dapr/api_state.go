...
func (a apiImpl) SaveState(storeName, key string, value interface{}) error {
	...
	err = daprClient.SaveState(a.ctx, normalize(storeName), key, data, nil)
	if err != nil {
...
func (a apiImpl) GetState(storeName, key string) ([]byte, error) {
	...
	// IMPORTANT: daprClient是全局的连接, 不能关闭
	item, err := daprClient.GetState(a.ctx, normalize(storeName), key, nil)
	if err != nil {
...
func (a apiImpl) GetBulkState(storeName string, keys any) (map[string][]byte, error) {
	...
	// IMPORTANT: daprClient是全局的连接, 不能关闭
	items, err := daprClient.GetBulkState(a.ctx, normalize(storeName), strKeys, nil, 100)
	if err != nil {
...
func (a apiImpl) DeleteState(storeName, key string) error {
	...
	// IMPORTANT: daprClient是全局的连接, 不能关闭
	//defer daprClient.Close()
	err = daprClient.DeleteState(a.ctx, normalize(storeName), key, nil)
	if err != nil {
