...
func (a apiImpl) Lock(lockStore, lockOwner, resource string, expiryInSeconds int) error {
	...
	resp, err := daprClient.TryLockAlpha1(a.ctx, namespace.Encapsulate(lockStore), &client.LockRequest{
		LockOwner:       lockOwner,
		ResourceID:      resource,
...
func (a apiImpl) Unlock(lockStore, lockOwner, resource string) error {
	...
	resp, err := daprClient.UnlockAlpha1(a.ctx, namespace.Encapsulate(lockStore), &client.UnlockRequest{
		LockOwner:  lockOwner,
		ResourceID: resource,
	})
	if err != nil {
